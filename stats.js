...
function aggregateHourlySummary(hourlyByDay) {
    ...
    for (let h = 9; h <= 17; h++) {
        const perDayTotals = dates.map(d => hourlyByDay.matrix[d][h].totale),
            perDayAdulti = dates.map(d => hourlyByDay.matrix[d][h].adulti),
            perDayBamb = dates.map(d => hourlyByDay.matrix[d][h].bambini),
            perDayRev = dates.map(d => hourlyByDay.matrix[d][h].revenue);

        const totalArrivals = perDayTotals.reduce((s, v) => s + v, 0),
            totalAdulti = perDayAdulti.reduce((s, v) => s + v, 0),
            totalBamb = perDayBamb.reduce((s, v) => s + v, 0),
            totalRev = perDayRev.reduce((s, v) => s + v, 0);
...
function computeOccupancyTimeline(R, bucketMinutes = 5) {
    ...
    for (const rec of R) {
        const start = rec.dt.getTime(), end = start + PERMANENZA_MIN * 60 * 1000;
...
function computeKPIs() {
    const totClients = filteredRecords.length,
        totRevenue = filteredRecords.reduce((s, r) => s + r.price, 0),
        dailyCounts = aggregatedDaily.map(d => d.n);
    ...
    const occ = computeOccupancyTimeline(filteredRecords, 5),
        peakCount = occ.counts.length ? Math.max(...occ.counts) : 0,
        peakKey = occ.keys[occ.counts.indexOf(peakCount)] || "-";
    ...
    // **Nuovo KPI: Incasso per cliente**
    const revenuePerClient = totClients > 0 ? (totRevenue / totClients) : 0;
    ...
    return {
        ...
        predStr,
        revenuePerClient: revenuePerClient.toFixed(2) // aggiunto
    };
}
...
function renderKPIs() {
    ...
    document.getElementById("kpi-revenue-per-client").textContent = toEur(k.revenuePerClient); // nuovo KPI
...
function renderHourlyTable() {
    ...
    const top = aggregatedHourly.summary.sort((a, b) => b.totalArrivals - a.totalArrivals)
        .slice(0, 8)
        .map((r, i) => `${i + 1}. ${pad2(r.hour)}:00 — ${r.totalArrivals}`)
        .join("<br/>");
